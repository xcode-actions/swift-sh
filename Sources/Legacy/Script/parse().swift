...
func parse(_ line: String, from input: Script.Input) throws -> ImportSpecification? {
	...
	/* Doesn’t look like an import line, we have to silently ignore it, even though it could be a typo or whatever which is not useful to our user,
	 *  but at the end of the day we’re a hack waiting for a real API in Swift itself that can then error properly
