...
/* TODO: Should we update packages? maybe in background when running scripts. */
...
private func run<T>(reader: StreamReader, input: Input, arguments: T) throws -> Never where T: Collection, T.Element == String {
	 ...
	 * Since any line starting with import that is not in a comment must be at file-scope or it is invalid Swift we just look for that.
	 * TODO: If we are inside a comment block, know that, and wait for end of comment block.
	 * TODO: May need to parse `#if os()` etc. too, which may mean we should just use SourceKitten and do a proper parse.
	 * TODO: Well also could have an import structure where is split over multiple lines with semicolons. So maybe parser? */
