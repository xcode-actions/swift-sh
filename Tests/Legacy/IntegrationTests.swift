...
class RunIntegrationTests: XCTestCase {
...
#if swift(>=5)
#else
		/* Sleep or race condition bug in SwiftPM 4.2 causes changes to be ignored if the same script name is executed twice in quick succession. */
		usleep(500_000)
#endif
	}
	...
	func testTwoScriptsSameNameWork() throws {
		/* In the same temporary directory we create two directories each containig a script of the same name but slightly different bodies.
		 * If swift-sh cache is not disambiguating based on full path the the second script will not be built
		 *  and we will see the output of the first when executing the second. */
...
class CleanIntegrationTests: XCTestCase {
	...
	func testCanCleanSpecificScripts() throws {
		/* In the same temporary directory we create two directories each containing a script of the same name.
		 * After executing the new scripts they will both be built in different directories inside of the build directory.
		 * Running clean on the first script will remove its build directory but leave the second. */
		try Path.mktemp{ tmpdir -> Void in
...
class EjectIntegrationTests: XCTestCase {
	...
	func testFilenameDirectoryClash() throws {
		 ...
		 *  this is a filename clash on macOS with its case insensitive filesystem and we still should *work*.
	...
	func testWorksIfSymlinkBecomesBroken() {
		/* Creates two scripts for the same cache location thus when the first is complete the `main.swift` symlink becomes broken.
